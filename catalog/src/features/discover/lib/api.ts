...
import { getTenantFullUrl } from '@umkm/shared';
...
export async function fetchTenantSitemap(
  ...
  const res = await fetch(`${API_URL}/sitemap/tenants/paginated?page=${page}&limit=${limit}`);
 ...
 */
export async function fetchTenantBySlug(slug: string): Promise<TenantDetail | null> {
  try {
...
export async function fetchAllTenants(
  ...
  const tenantDetails = await Promise.all(tenantSlugs.map((item) => fetchTenantBySlug(item.slug)));
