...
import { Search, X, TrendingUp, Clock, ArrowRight, Store, type LucideIcon } from 'lucide-react';
import { Input } from '@umkm/shared/ui';
import { Button } from '@umkm/shared/ui';
import { Badge } from '@umkm/shared/ui';
import { cn } from '@umkm/shared/utils';
import { getCategoryList } from '@umkm/shared/config';
...
export function DiscoverSearch({
  ...
  const suggestions = useMemo((): SearchSuggestion[] => {
    ...
    if (queryLower) {
      ...
      const popularCategories = categories.slice(0, 4).map(
        (cat): SearchSuggestion => ({
          type: 'category',
          label: cat.label,
          value: cat.label,
        ...
        <div className="relative flex items-center">
          ...
          <Input
            ...
            className={cn(
              ...
              isActive && ['bg-background', 'border-primary/25', 'shadow-sm']
            )}
          />
...
function SuggestionItem({ suggestion, Icon, isSelected, onClick }: SuggestionItemProps) {
  return (
    ...
    >
      ...
      >
        <Icon className="h-4 w-4" style={{ color: suggestion.color || 'currentColor' }} />
      </div>
