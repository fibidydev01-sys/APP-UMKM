...
import { Store, Package, ArrowRight, AlertCircle, Loader2, ArrowLeft } from 'lucide-react';
...
import { CATEGORY_CONFIG } from '@umkm/shared/config';
import {
...
function CategoryHero({ category, categoryKey, tenantCount }: CategoryHeroProps) {
  ...
  const label =
    category?.label || categoryKey.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
...
export function CategoryPageClient({
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      {/* Header */}
      <DiscoverHeader onSearch={handleSearch} searchQuery={searchQuery} />

      <main className="flex-1">
        {/* Category Hero - ✅ Pass category safely */}
        <CategoryHero category={category} categoryKey={categoryKey} tenantCount={tenants.length} />
