      ...
      <div className="container px-4 py-8">
        ...
        <div className="mb-6">
          <StoreBreadcrumb
            ...
            items={[{ label: 'Produk', href: `/store/${slug}/products` }, { label: product.name }]}
          />
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Bagikan produk ini:</span>
              <SocialShare
                url={productUrl}
                title={`${product.name} - ${tenant.name}`}
                description={product.description || `Beli ${product.name} di ${tenant.name}`}
                variant="buttons"
              />
            </div>
