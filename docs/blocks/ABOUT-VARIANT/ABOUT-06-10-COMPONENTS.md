...
export function About06({ title, subtitle, content, image, features = [] }: About06Props) {
  const galleryImages = [
    image,
...
export function About07({ title, subtitle, content, image, features = [] }: About07Props) {
  const chromaItems = [
    ...(image ? [{ image, title }] : []),
    ...features.map(f => ({
      image: f.icon || '',
...
export function About08({ title, subtitle, content, image, features = [] }: About08Props) {
  const posterImages = [
    image,
    ...features.map(f => f.icon).filter(Boolean)
  ].filter(Boolean) as string[];
...
export function About09({ title, subtitle, content, image, features = [] }: About09Props) {
