...
5 About section variants dengan React Bits integration (Component & Text Animations ONLY), **SLATE THEME** (blue-gray modern) dari shadcn UI, experimental combinations, dan unique layouts.
...
export function About41({ title, subtitle, content, image, features = [] }: About41Props) {
  const navItems = [
    { label: 'Home', href: '/' },
...
export function About42({ title, subtitle, content, image, features = [] }: About42Props) {
  const menuItems = [
  ...
  return (
    <section id="about" className="relative py-20 overflow-hidden bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 min-h-screen">
      {/* Pixel Trail Cursor */}
...
export function About43({ title, subtitle, content, image, features = [] }: About43Props) {
  return (
...
export function About44({ title, subtitle, content, image, features = [] }: About44Props) {
...
export function About45({ title, subtitle, content, image, features = [] }: About45Props) {
  const trailImages = [
