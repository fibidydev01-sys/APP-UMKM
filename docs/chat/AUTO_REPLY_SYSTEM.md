...
| `{{tracking_link}}` | LINK | Order Status, Payment | Order tracking URL |
...
const replacedMessage = template
  ...
  .replace(/\{\{tracking_link\}\}/g, order.trackingUrl);
```
...
- **{{tracking_link}}**: Generated when order is created
...
Cek detail: https://toko.com/track/xyz123
```
